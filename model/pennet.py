...
class AtnConv(nn.Module):
  ...
  def forward(self, x1, x2, mask=None): # X5 X6
    ...
    Args:
        x1: low-level feature maps with larger resolution. 有着更大分辨率的低层特征
        x2: high-level feature maps with smaller resolution.   有着更小分辨率的高层特征
        mask: Input mask, 1 indicates holes. 输入掩码，1表示空洞。
    ...
    # 从低级特征图x1提取patches  with stride and rate  ---- func of f
    kernel = 2*self.rate
    # sampling
    ...
    # raw_w 形成的是经过F函数之后下采样
    raw_w_groups = torch.split(raw_w, 1, dim=0) # B*[1,HW,C,K,K]

    # split high-level feature maps x2 for matching   --- func of g
    f_groups = torch.split(x2, 1, dim=0)  # B * [1,C,H,W]
    # 从x2提取patches 作为 weights of filter
    ...
    # w形成的是经过g函数之后下采样
    w_groups = torch.split(w, 1, dim=0) # B * [1,HW,C,K,K]
    ...
    m = m.mean([2,3,4]).unsqueeze(-1).unsqueeze(-1) # (B, HW, 1, 1)
    mm = m.eq(0.).float() # (B, HW, 1, 1)
    mm_groups = torch.split(mm, 1, dim=0) # B * [1,HW,1,1]

    y = []
    scale = self.softmax_scale # 10
    padding = 0 if self.ksize==1 else 1  # padding :0
    # wi 高层特征, xi 低层特征
    for xi, wi, raw_wi, mi in zip(f_groups, w_groups, raw_w_groups, mm_groups):
      '''
      O => output channel as a conv filter  输出通道为conv滤波器
      I => input channel as a conv filter   输入通道为conv滤波器
      xi : separated tensor along batch dimension of front;  沿前面的批量维数分离张量
      wi : separated patch tensor along batch dimension of back;  沿背的批量维分离的块张量
      raw_wi : separated tensor along batch dimension of back;   沿批次维数分离的张量
      '''
      # matching based on cosine-similarity 基于余弦相似度的匹配
      wi = wi[0] # wi : (1,HW,C,K,K) => (HW,C,K,K)
      escape_NaN = torch.FloatTensor([1e-4])
      if torch.cuda.is_available():
        escape_NaN = escape_NaN.cuda()
      # normalize 
      wi_normed = wi / torch.max(torch.sqrt((wi*wi).sum([1,2,3],keepdim=True)), escape_NaN) # (HW,C,K,K)
...
def extract_patches(x, kernel=3, stride=1):
  if kernel != 1:
    x = nn.ZeroPad2d(1)(x) # 四周加上1的补丁
  x = x.permute(0, 2, 3, 1) # [B,H,W,C]
  all_patches = x.unfold(1, kernel, stride).unfold(2, kernel, stride)  # all : [16,4,4,512,4,4]
  # all_patches = x.unfold(1, kernel, stride)
  # all_patches = all_patches.unfold(2, kernel, stride)
  return all_patches
