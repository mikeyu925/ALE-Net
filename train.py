...
def main_worker(gpu, ngpus_per_node, config):
  ...
  # 'release_model/pennet_dtd_square256'
  ...
  if (not config['distributed']) or config['global_rank'] == 0:
    os.makedirs(config['save_dir'], exist_ok=True) # 创建文件夹
...
if __name__ == "__main__":
  ...
  # loading configs 
  config = json.load(open(args.config)) # 加载配置
  if args.mask is not None:
    config['data_loader']['mask'] = args.mask # 设置加载器的掩码mask
