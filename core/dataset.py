...
class Dataset(torch.utils.data.Dataset):
  ...
  def __init__(self, data_args, debug=False, split='train', level=None):
    ...
    self.data = [os.path.join(data_args['zip_root'], data_args['name'], i)  # 拼接路径 ['datasets\\dtd\\dtd/banded_0002.jpg', 'datasets\\dtd\\dtd/banded_0004.jpg'.....]
      for i in np.genfromtxt(os.path.join(data_args['flist_root'], data_args['name'], split+'.flist'), dtype=np.str, encoding='utf-8')]
    self.mask_type = data_args.get('mask', 'pconv') # mask_type: square
    if self.mask_type == 'pconv': # fasle
    ...
    if split == 'train':
      self.data = self.data * data_args['extend']# 为什么要乘以20呢
      shuffle(self.data)  # 进行随机排序
